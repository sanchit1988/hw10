...
function decide_response(user_said) {
  ...
  if (user_said.toLowerCase().includes("weather in") && user_said.toLowerCase().includes("and")) {
    ...
    response = "Shucks, another user!! Alright, I am Homer, I can tell you about the weather";
    }
    else if (user_said.toLowerCase().includes("hey")) {
    response = "I told you to say Hi";
    }
    else {
...
function speak(text, callback) {
  ...
  u.voice = speechSynthesis.getVoices().filter(function(voice) { return voice.name == "Daniel"; })[0]; //pick a voice
